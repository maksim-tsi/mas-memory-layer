...
class TestTierTools:
    ...
    async def test_l2_search_facts_emits_tool_and_retriever_spans(self, mocker):
        l2_tier = mocker.Mock()
        l2_tier.ciar_threshold = 0.6
        l2_tier.search_facts = mocker.AsyncMock(
            return_value=[
        ...
        assert payload["results_count"] == 1
        assert [entry["name"] for entry in fake_tracer.started] == [
            "yaam.tool.l2_search_facts",
            "yaam.retriever.l2",
        ]
        tool_span = fake_tracer.started[0]["span"]
        retriever_span = fake_tracer.started[1]["span"]
        assert tool_span.attributes["openinference.span.kind"] == "TOOL"
        assert retriever_span.attributes["openinference.span.kind"] == "RETRIEVER"
        assert retriever_span.attributes["session.id"] == "session-123"
        assert "retrieval.documents" in retriever_span.attributes

    async def test_l3_query_graph_emits_tool_span(self, mocker):
        l3_tier = mocker.Mock()
        l3_tier.query_graph = mocker.AsyncMock(return_value=[{"episode_id": "episode-1"}])
        memory_system = type("MemorySystem", (), {"l3_tier": l3_tier})()
        runtime = MockToolRuntime(context=MockContext(memory_system=memory_system), state={})
        ...
        assert payload["results_count"] == 1
        assert [entry["name"] for entry in fake_tracer.started] == ["yaam.tool.l3_query_graph"]
        tool_span = fake_tracer.started[0]["span"]
        assert tool_span.attributes["openinference.span.kind"] == "TOOL"
        assert tool_span.attributes["yaam.graph.template"] == "get_container_journey"

    async def test_l3_search_episodes_emits_tool_and_retriever_spans(self, mocker):
        llm_client = mocker.Mock()
        llm_client.get_embedding = mocker.AsyncMock(return_value=[0.1, 0.2, 0.3])
        ...
        assert payload["results_count"] == 1
        assert [entry["name"] for entry in fake_tracer.started] == [
            "yaam.tool.l3_search_episodes",
            "yaam.retriever.l3",
        ]
        retriever_span = fake_tracer.started[1]["span"]
        assert retriever_span.attributes["input.value"] == "customs port delay"
        assert "retrieval.documents" in retriever_span.attributes

    async def test_l4_search_knowledge_emits_tool_and_retriever_spans(self, mocker):
        document = KnowledgeDocument(
            knowledge_id="knowledge-1",
            title="Delay pattern",
            content="Customs delays increase when documentation arrives incomplete.",
