...
Этот движок отвечает за фильтрацию шума. Он анализирует входящий поток сообщений из L1, применяя логику **CIAR (Certainty-Impact-Age-Recency)**.

* *Семантические Входы:* Семантические модули более высокого уровня, такие как сегментация тем и извлечение фактов, формируют кандидаты в факты и оценивают исходные признаки `certainty` и `impact`.
* *Детерминированное Правило Продвижения:* Сам CIAR-скоринг не является отдельным LLM-вызовом. Он детерминированно вычисляет итоговую значимость на основе `certainty`, `impact`, временного затухания и усиления от повторных обращений. Только факты, превысившие порог значимости (Significance Threshold) и содержащие новую информацию (Delta), "продвигаются" (promoted) в уровень L2. Это предотвращает засорение долгосрочной памяти приветствиями и техническим шумом.
