...
Status: CIAR-DEF-3 implemented and live-evaluated; `hybrid_gate` remains the recommended promotion default and `suppress_superseded` remains opt-in
Related:
...
| CIAR-DEF-3 | Complete live-evaluated | Refresh CIAR policy recommendation with Batch 1-7 evidence, fresh live LLM matrix, and Phoenix span export | Experiment owner | CIAR-RES-1, CIAR-REV-1, CIAR-REC-1, CIAR-SCEN-2, CIAR-SUP-3 | `docs/plan/2026-05-23-ciar-policy-recommendation-refresh-plan.md`, `docs/reports/2026-05-23-ciar-policy-recommendation-refresh-report.md`, `logs/ciar_challenge/ciar-policy-refresh-20260523.md`, `logs/ciar_challenge/ciar-policy-refresh-phoenix-summary-20260523.json` | Six required-health live runs classified as `policy_evidence`; recommendation keeps `hybrid_gate` as promotion default and keeps `suppress_superseded` opt-in pending stronger current-fact preservation evidence |
...
| CIAR-SCEN-1 | Complete locally | Repair `repeated_correction` challenge scenario reliability | Experiment owner | CIAR-SUP-2, CIAR-REG-1 | `docs/plan/2026-05-23-ciar-repeated-correction-repair-plan.md`, `tests/scripts/test_ciar_challenge_experiment.py`, focused dry-run artifacts | Focused dry `hybrid_gate+suppress_superseded` evidence promotes the latest Long Beach route and suppresses prior Oakland and Los Angeles route facts |
| CIAR-SCEN-2 | Complete locally with live evidence | Improve live `repeated_correction` observability | Experiment owner | CIAR-SCEN-1, CIAR-REG-1 | `docs/plan/2026-05-23-ciar-live-repeated-correction-robustness-plan.md`, `docs/reports/2026-05-23-ciar-live-repeated-correction-robustness-report.md`, `logs/ciar_challenge/ciar-live-repeated-correction-robustness-20260523.md` | Two required-health live runs classified as `policy_evidence`; each promoted the `repeated_correction` segment, extracted six facts, promoted current Long Beach operational facts, and suppressed three prior/duplicate route facts |
| CIAR-SUP-3 | Complete locally | Add focused dry suppression evaluation for canonical contradiction scenarios | Experiment owner | CIAR-SCEN-1, CIAR-REG-1 | `docs/plan/2026-05-23-ciar-focused-suppression-evaluation-plan.md`, `scripts/experiments/analyze_ciar_policy_runs.py`, `tests/scripts/test_ciar_policy_analysis.py`, `docs/reports/2026-05-23-ciar-focused-suppression-evaluation-report.md` | Analyzer reports focused suppression metrics for `contradiction_update` and `repeated_correction`; dry matrix shows suppression only under `suppress_superseded` |
| CIAR-RES-1 | Complete locally | Tighten conversational residue handling for assistant acknowledgements and commitments | Memory policy owner | CIAR-REG-1, CIAR-SCEN-2 | `docs/plan/2026-05-23-ciar-residue-tightening-plan.md`, `scripts/experiments/analyze_ciar_policy_runs.py`, `tests/memory/engines/test_promotion_engine.py`, `tests/scripts/test_ciar_challenge_experiment.py`, `tests/scripts/test_ciar_policy_analysis.py` | Focused dry evidence keeps durable operational facts promoted while assistant/chatter residue is review-only under `hybrid_gate` |
| CIAR-REV-1 | Complete locally | Make speculative and assistant-inferred claims review-only evidence | Memory policy owner | CIAR-REG-1, CIAR-RES-1 | `docs/plan/2026-05-23-ciar-speculative-claims-review-only-plan.md`, `scripts/experiments/analyze_ciar_policy_runs.py`, `tests/memory/engines/test_promotion_engine.py`, `tests/scripts/test_ciar_challenge_experiment.py`, `tests/scripts/test_ciar_policy_analysis.py` | Focused dry evidence extracts `speculative_claim` and `assistant_inferred` facts, marks them review-only, and keeps positive controls promoted |
| CIAR-REC-1 | Complete locally | Prevent recency/access reinforcement from storing weak base evidence by itself | Memory policy owner | CIAR-REG-1, CIAR-REV-1 | `docs/plan/2026-05-23-ciar-recency-access-guardrail-plan.md`, `docs/reports/2026-05-23-ciar-recency-access-guardrail-report.md`, `logs/ciar_challenge/ciar-recency-access-guardrail-20260523.md`, `tests/memory/engines/test_promotion_engine.py`, `tests/scripts/test_ciar_challenge_experiment.py`, `tests/scripts/test_ciar_policy_analysis.py` | Focused dry evidence marks `access_reinforced_low_signal` review-only under `hybrid_gate`, while positive controls still promote and CIAR formula semantics remain unchanged |
